...
def _combine_middlewares(middlewares):
    def empty_middleware(next): return lambda action: next(action)
    def combine_middlewares(
        first, second): return lambda next: first(second(next))
...
class Store():
    ...
    def act(self, action: Action):
        def act(action): return action.act(self.state)
